...
def main():
    ...
    #communication
    clientsocket.send(store.encode('utf-8'))
    ...
    if reply == "Success":
        ask='available'
        clientsocket.send(ask.encode('utf-8'))
        print ("send",ask)
        ...
        if result == 'notavailable':
            ...
            print ('Phase2: The requested appointment is available and reserved to Patient2. The assigned doctor name & port number is', result)
            clientsocket.close()
            #--------------Phase3----------------
